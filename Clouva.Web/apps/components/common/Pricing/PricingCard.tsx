...
const PricingCard: React.FC<PricingCardProps> = ({ sub }) => {
    ...
    return (
        <div
            className={`flex flex-col justify-between space-y-3 border rounded-[1rem] w-full md:max-w-[380px] p-5 ${true ? 'bg-white border-[#e5e7eb]' : 'bg-black text-white border-[#e5e7eb]'}`}
        >
            <p>
                ...
                <div className="space-y-1">
                    {sub.features[langKey].map((feat, index) => (
                        <div className="flex items-center gap-4" key={index}>
                            <div className={`${true ? 'bg-black' : 'bg-white'} p-[2px] rounded-full`}>
                                <Check fill={true ? '#fff' : '#000'} size={13} />
                            </div>
            ...
            <GUIButton
                className={`!text-[15px] !h-[2.5rem] !mt-3 ${true ? '!cursor-not-allowed' : '!bg-white !text-black hover:opacity-80'}`}
                disabled={!canCheckout}
                onClick={canCheckout ? handleCheckout : undefined}
            >
                {true ? t('label.pricing-current') : t('label.pricing-get-started')}
            </GUIButton>
        </div>
    );
};
